...
class DatabaseLedger:
    ...
    def add_trade(self, pair, trade_type, units_1, units_2, pair_1_open, pair_2_open):
        self.open_db()
        date_open = datetime.datetime.today()
        new_trade = (pair, trade_type, date_open, "", "OPEN", units_1, units_2, pair_1_open, pair_2_open, "", "")

        self.c.execute("""INSERT INTO tx_ledger VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", new_trade)
        self.conn.commit()
        self.conn.close()

    # Finds the open trade and closes it
    ...
    # Get the activity for a pair
    #def pull_activity_list(self, pair):
    #    self.open_db()
    #    self.c = self.conn.cursor()
    #    self.c.execute("""SELECT * FROM activity_log WHERE pair_name = ?""", (pair,))
    #    items = self.c.fetchall()

    #    activity_list = []
    #    for item in items:
    #        # (pair, z_score, pair_1_price, pair_2_price, date)
    #        activity_list.append((item[0], item[1], item[2], item[3], item[4]))
