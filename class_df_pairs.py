...
class FindPairs:
    ...
    def determine_betas(self):
        ...
        file_path = os.path.join(file_dir, "Analysis/analysis_values.json")

        with open(file_path, "r") as f:
            self.betas = json.load(f)

    # Get data from local file instead of the API, useful for training data
    def use_data(self):
        self.data_df = pd.read_csv("ETH_BTC_train.csv").drop("Unnamed: 0", axis=1)
        self.df_array.append(self.data_df)

    # Get data from API, useful if you don't have training data or for test data
    def run_pairs(self):
    ...
    def feature_gen(self, pair1, pair2="BTC"):
        ...
        # Build df for train or test, this will determine the size of the dataframe and when it is taken
    ...
    # Generates statistical information for the cointegration spread
    def find_stats(self):
    ...
    # API to generate testing data - 1 month
    ...
    # API to generate training data - 2 years
