...
def run_quant():
    ...
    # Generate data or use local training data
    if gen_data:
        pair_data.run_pairs()
    else:
        pair_data.use_data()

    # Go through df to run strategy - This is done if you have more than 1 trading pairs
    for df in pair_data.df_array:
        # Setup dataframe columns in order
        df = df.reindex(['ETH Close', 'BTC Close', 'Coint Spread', 'Mean', 'STD', 'Z-Score', 'Z-Score Lag 1', 'Z-Score Lag 2',
        ...
        for x in range(1, len(df)):
            # Extracts data row by row including current row and future row
            # The future row is needed to try account for bid/ask spread and data snooping bias
            pair_1, pair_2, spread, mean, std, zscore, zscore_1, zscore_2, *lag_list = df.iloc[x-1]
            pair_1_, pair_2_, spread_, mean_, std_, zscore_, zscore_1_, zscore_2_, *_lag_list = df.iloc[x]

            # Extracting data and reporting it
            pair_1_name = "ETH"
            pair_2_name = "BTC"
            ...
            # Check for open trades and close them if needed based on the z-score
            trade_actions = db_ledger.find_open_trades(pair_name)

            # Closing any open trades if needed
            if trade_actions:
                ...
                if item_trade == "Long":
                    if zscore <= 0.2:
                        db_ledger.close_trade(pair_name, (pair_1_ + pair_1)/2, (pair_2 + pair_2_)/2)
            ...
            trade_option = make_decision(zscore)

            # Actions trading option, checks bank balance and trade counts and executes the trade
            if trade_option:
                # Trade count limits
                if not limit_trades:
                ...
                if len(trade_actions) < trade_counts[pair_1_name]:
                    ...
                    # Determine the amount of units to purchase
                    pair_1_trade = round(
